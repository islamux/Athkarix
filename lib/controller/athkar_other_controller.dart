...
class AthkarOtherControllerImp extends BaseAthkarController {
  // Instance from PageController to go to next page in pageview builder
  PageController pageControllerO = PageController();
  
  // The loaded adhkar list from JSON
  List<dynamic>? adhkarOtherList;

  // Provide the specific PageController for this page
  @override
  PageController get pageController => pageControllerO;

  // Provide the completion message for this page
  @override
  String get completionMessage => 'أنهيت الأذكار الأخرى !';

  // Provide the specific data list for this page  
  @override
  List<dynamic> get dataList => adhkarOtherList ?? [];
  ...
  Future<void> loadAdhkarData() async {
    final String jsonString = await rootBundle.loadString('lib/core/data/json/adhkar_other.json');
    adhkarOtherList = json.decode(jsonString);
    update();
  }
  ...
  List<int> get maxPageCounters {
    ...
    if (adhkarOtherList != null && adhkarOtherList!.isNotEmpty) {
      return adhkarOtherList!.map((item) => item['count'] as int? ?? 1).toList();
    }
